...
function NewProductScreen() {
  ...
  const { slug } = params;
  const [{ loading, error, newproduct, loadingCreateReview }, dispatch] =
    useReducer(reducer, {
      newproduct: [],
      loading: true,
      error: '',
  ...
  const addToCartHandler = async () => {
    const existItem = cart.cartItems.find((x) => x._id === newproduct._id);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `/api/newproducts/${newproduct._id}/reviews`,
        ...
        <div>
          <Row>
            <Col md={6}>
              <img
                className="img-large"
                src={selectedImage || newproduct.image}
                alt={newproduct.name}
              ></img>
            </Col>
            <Col md={3}>
              <ListGroup variant="flush">
                ...
                <ListGroup.Item>
                  Pirce : Ksh.{numberWithCommas(newproduct.price)}
                </ListGroup.Item>
                <ListGroup.Item>
                  <Row xs={1} md={2} className="g-2">
                    {[newproduct.image, ...newproduct.images].map((x) => (
                      <Col key={x}>
                        <Card>
                          <Button
                            className="thumbnail"
                ...
                <Card.Body>
                  <ListGroup variant="flush">
                    <ListGroup.Item>
                      <Row>
                        <Col>Price:</Col>
                        <Col>Ksh. {numberWithCommas(newproduct.price)}</Col>
                      </Row>
                    </ListGroup.Item>
                    <ListGroup.Item>
                      <Row>
